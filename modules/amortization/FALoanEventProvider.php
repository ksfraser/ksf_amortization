...
class FALoanEventProvider implements LoanEventProviderInterface {
    ...
    public function insertLoanEvent(LoanEvent $event): void {
        $sql = "INSERT INTO loan_events (loan_id, event_type, event_date, amount, notes) VALUES ('" .
    ...
    public function getLoanEvents(int $loan_id): array {
        $sql = "SELECT * FROM loan_events WHERE loan_id = '" . $this->db->escape($loan_id) . "' ORDER BY event_date ASC";
    ...
    public function updateLoanEvent(LoanEvent $event): void {
        $sql = "UPDATE loan_events SET event_type = '" . $this->db->escape($event->event_type) .
    ...
    public function deleteLoanEvent(int $event_id): void {
        $sql = "DELETE FROM loan_events WHERE id = '" . $this->db->escape($event_id) . "'";
        $this->db->query($sql);
    }
}
