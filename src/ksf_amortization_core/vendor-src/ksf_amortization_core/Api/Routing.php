...
class ApiRouter
{
    ...
    public function dispatch(string $method, string $path, array $data = []): ApiResponse
    {
        try {
            ...
            // Replacing match with if/else for PHP 7.3 compatibility
            // LOAN ROUTES
            if ($this->matches($segments, ['api', 'v1', 'loans']) && $method === 'GET') {
                return $this->loanController->list($data);
            } elseif ($this->matches($segments, ['api', 'v1', 'loans']) && $method === 'POST') {
                return $this->loanController->create($data);
            } elseif ($this->matchesWithId($segments, ['api', 'v1', 'loans']) && $method === 'GET') {
                return $this->loanController->get((int)end($segments));
            } elseif ($this->matchesWithId($segments, ['api', 'v1', 'loans']) && $method === 'PUT') {
                return $this->loanController->update((int)end($segments), $data);
            } elseif ($this->matchesWithId($segments, ['api', 'v1', 'loans']) && $method === 'DELETE') {
                return $this->loanController->delete((int)end($segments));
            }
            // SCHEDULE ROUTES
            elseif ($this->matches($segments, ['api', 'v1', 'loans', null, 'schedules']) && $method === 'GET') {
                return $this->scheduleController->list((int)$segments[3], $data);
            } elseif ($this->matches($segments, ['api', 'v1', 'loans', null, 'schedules']) && $method === 'POST') {
                return $this->scheduleController->generate((int)$segments[3], $data);
            } elseif ($this->matches($segments, ['api', 'v1', 'loans', null, 'schedules']) && $method === 'DELETE') {
                return $this->scheduleController->deleteAfterDate((int)$segments[3], $data);
            }
            // EVENT ROUTES
            elseif ($this->matches($segments, ['api', 'v1', 'loans', null, 'events']) && $method === 'GET') {
                return $this->eventController->list((int)$segments[3], $data);
            } elseif ($this->matches($segments, ['api', 'v1', 'loans', null, 'events']) && $method === 'POST') {
                return $this->eventController->record((int)$segments[3], $data);
            } elseif ($this->matchesWithEventId($segments, ['api', 'v1', 'loans', null, 'events']) && $method === 'GET') {
                return $this->eventController->get((int)$segments[3], (int)end($segments));
            } elseif ($this->matchesWithEventId($segments, ['api', 'v1', 'loans', null, 'events']) && $method === 'DELETE') {
                return $this->eventController->delete((int)$segments[3], (int)end($segments));
            }
            // Default: Not found
            else {
    ...
    private function parsePath(string $path): array
    {
        ...
        return array_filter($segments, function($s) { return !empty($s); });
    }
