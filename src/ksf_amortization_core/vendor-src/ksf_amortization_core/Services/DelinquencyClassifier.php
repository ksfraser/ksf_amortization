...
class DelinquencyClassifier
{
    ...
    public function identifyPaymentPattern(Loan $loan): array
    {
        ...
        $onTimeCount = count(array_filter($history, function($r) { return $r['status'] === 'on_time'; }));
        $lateCount = count(array_filter($history, function($r) { return $r['status'] === 'late'; }));
        $missedCount = count(array_filter($history, function($r) { return $r['status'] === 'missed'; }));
    ...
    private function determineTrend(array $history): string
    {
        ...
        $recentLateCount = count(array_filter($recent, function($r) { return $r['status'] === 'late'; }));
        $olderLateCount = count(array_filter($older, function($r) { return $r['status'] === 'late'; }));
