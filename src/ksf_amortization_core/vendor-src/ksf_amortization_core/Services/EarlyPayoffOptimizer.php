...
class EarlyPayoffOptimizer
{
    ...
    public function runMonteCarloSimulation(Loan $loan, float $baseRate, float $volatility, int $simulations, float $payoffStrategy): array
    {
        ...
        $variance = array_reduce($payoffMonths, function($carry, $val) use ($mean) { return $carry + pow($val - $mean, 2); }, 0) / count($payoffMonths);
        ...
        return [
            ...
            'scenarios' => array_map(function($m) { return ['payoff_months' => $m]; }, array_slice($payoffMonths, 0, 10)),
        ];
    }
