...
class EventRecordingService
{
    ...
    public function getEventsByType(
        ...
        $filtered = array_filter(
            $allEvents['events'] ?? [],
            function($event) use ($eventType) { return $event->event_type === $eventType; }
        );
    ...
    public function getEventsByDateRange(
        ...
        $filtered = array_filter(
            $allEvents['events'] ?? [],
            function($event) use ($startDate, $endDate) { return $event->event_date >= $startDate && $event->event_date <= $endDate; }
        );
